...
iter=2;
while ~isempty(C)
    rho = zeros([1 size(C,1)]); % the following nested for loop computes the correlation, rho(j),
    ...
    [~,ind]=max(rho); % this line selects the feature which maximizes the new cost
